...
select, button { width:100%; margin-top:.55rem; padding:.55rem; border-radius:7px; border:1px solid #405878; background:#10151e; color:#e8eef9; font:inherit; } button { background:#2570c9; border:0; cursor:pointer; } #inputStatus { display:block; margin-top:.6rem; color:#9fb0c9; }
...
<dt>Format</dt><dd id="format">—</dd><dt>Input queue</dt><dd id="queue">—</dd><dt>Dropped frames</dt><dd id="dropped">—</dd><dt>Buffered audio</dt><dd id="buffer">—</dd>
...
function snapshot(s) { const speaking = s.vad_state === 'SPEAKING'; const state=byId('state'); state.textContent=`● ${s.vad_state}`; state.className=speaking?'speaking':'listening';
 ...
 byId('turn').textContent=s.turn_state; byId('format').textContent=`${s.sample_rate} Hz · ${s.frame_duration_ms} ms`; byId('queue').textContent=s.audio_queue_size; byId('dropped').textContent=s.frames_dropped; byId('buffer').textContent=`${s.buffered_audio_seconds}s`; }
